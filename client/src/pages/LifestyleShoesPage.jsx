...
const LifestyleShoesPage = () => {
    const [lifestyleShoes, setLifestyleShoes] = useState([]);
    ...
    useEffect(() => {
        const fetchLifestyleShoes = async () => {
            try {
                const response = await fetch('/api/lifestyle-shoes');
                if (!response.ok) {
                    throw new Error('Failed to fetch lifestyle shoes');
                }
                const data = await response.json();
                setLifestyleShoes(data);
            } catch (err) {
    ...
    const filteredShoes = lifestyleShoes.filter(shoe => {
        return (
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <div className="container mx-auto px-4 py-10">
                ...
                {loading && <p className="text-center text-gray-500">Loading lifestyle shoes...</p>}
                {error && <p className="text-center text-red-500">{error}</p>}

                {!loading && !error && (
                    <div className="max-w-7xl mx-auto">
                        {filteredShoes.length === 0 ? (
                            <p className="text-center text-gray-500">No lifestyle shoes found.</p>
                        ) : (
...
export default LifestyleShoesPage;
